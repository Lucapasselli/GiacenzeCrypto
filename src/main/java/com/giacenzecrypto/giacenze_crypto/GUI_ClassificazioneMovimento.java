...
public class GUI_ClassificazioneMovimento extends javax.swing.JDialog {
   ...
   //////////// //DSC -> Scambio Crypto Differito (Scambio crypto non simultaneo ma differito nel tempo) (Non Utilizzato per ora)
    ...
    //COMBOBOX DEPOSITI
    static final String CB_NESSUNASELEZIONE="- nessuna selezione / disassocia movimento -";
    ...
    static final String CB_DC_COLLATERALE="PRESTITO - SBLOCCO COLLATERALE (Chiusura del prestito)";
    ...
    //COMBOBOX PRELIEVI
    static final String CB_PC_CASHOUTCOMMISSIONE="CASHOUT / COMMISSIONI";
    static final String CB_PC_DONAZIONEFURTO="DONAZIONE, FURTO, BURN di Cripto-Attività";
    ...
    static final String CB_PC_LIQUIDAZIONEPRESTITO="PRESTITO - LIQUIDAZIONE FORZATA";
    static final String CB_PC_COLLATERALE="PRESTITO - MESSA A COLLATERALE";
    //ComboBox completo per singolo movimento
    ...
    public GUI_ClassificazioneMovimento(String ID) {
        ...
        if (tipomov.equalsIgnoreCase("PWN")) {
            ...
            ntipo = 1;
            if (riga[6].contains("Liquidazione"))ntipo=6;
            TransferNO();
            ...
            if (riga[6].contains("Collaterale"))ntipo=7;//Prelievo per messa a collaterale prestito
            TransferSI();
            ...
            if (riga[6].contains("Collaterale"))ntipo=8;//Deposito da sblocco collaterale prestito
            TransferSI();
        ...
        }else
        {
            ...
            //Anche se ho un solo elemento lo inserisco nella lista, verrà utilizzato successivamente
            IDsTrans.clear();
            IDsTrans.add(IDTrans);
    ...
    public GUI_ClassificazioneMovimento(Set<String> IDs) {
        ...
        if (!MovimentoMultiplo) {
            ...
            if (tipomov.equalsIgnoreCase("PWN")) {
                ...
                ntipo = 1;
                if (riga[6].contains("Liquidazione")) {
                    ntipo = 6;
                }
                TransferNO();
            } else if (tipomov.equalsIgnoreCase("PTW")) {
                ...
                if (riga[6].contains("Collaterale")) {
                    ntipo = 7;//Prelievo per messa a collaterale prestito
                }
                TransferSI();
            } else if (tipomov.equalsIgnoreCase("DTW")) {
                ntipo = 3;
                ...
                if (riga[6].contains("Collaterale")) {
                    ntipo = 8;//Deposito da sblocco collaterale prestito
                }
                TransferSI();
            } else if (tipomov.equalsIgnoreCase("DAI")) {
                ntipo = 1;
    ...
    private void Bottone_OKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Bottone_OKActionPerformed
        ...
        //Se scelgo nessuna selezione riporto tutto allo stato originale
        if (sceltaS.equals(CB_NESSUNASELEZIONE)){
            PulisciMovimentiAssociati(IDsTrans);
        ...
        if (IDTrans.split("_")[4].equalsIgnoreCase("DC")) {
            ...
            switch (sceltaS) {
                ...
                case CB_DC_TRASFERIMENTO -> {
                    descrizione = "TRASFERIMENTO TRA WALLET";
                    ModificaEffettuata=Crea_Trasferimenti(IDsTrans,descrizione,Note);
                    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
                    this.dispose();
                    return;
                }
            ...
            switch (sceltaS) {
                case CB_PC_CASHOUTCOMMISSIONE -> {
                    //può essere un movimento multiplo
                    ModificaEffettuata=Crea_CashoutCommissioni(IDsTrans,Note);
                    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
                    this.dispose();
                    return;
                }
                case CB_PC_DONAZIONEFURTO -> {
                    ModificaEffettuata=Crea_DonazioneFurto(IDsTrans,Note);
                    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
                    this.dispose();
                    return;
                }
                case CB_PC_TRASFERIMENTO -> {
                    descrizione = "TRASFERIMENTO TRA WALLET";
                    ModificaEffettuata=Crea_Trasferimenti(IDsTrans,descrizione,Note);
                    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
                    this.dispose();
                ...
                case CB_PC_LIQUIDAZIONEPRESTITO -> {
                    //Può essere un movimento multiplo
                    descrizione = "LIQUIDAZIONE PRESTITO";
                    dettaglio = "PCO - Liquidazione";
                    ClassificaMovimenti(IDsTrans,descrizione,dettaglio,Note,"",true);
                    ModificaEffettuata=true;
                    setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
                    this.dispose();
...
setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
    ...
    private Set<String> PulisciMovimentiAssociati(Set<String> IDsTr){
     Map<String,String[]> PartiCoinvolte=new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
              
        //Pulisco i vecchi movimenti
        for(String id:IDsTr){
        ...
        return RiportaTransazioniASituazioneIniziale(PartiCoinvolte);
    }
    
    private boolean Crea_Reward(Set<String> IDsTr, String Note) {
        //Può essere un movimento multiplo
        /*   Map<String,String[]> PartiCoinvolte=new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
        ...
        AppDialog.DialogResult result = AppDialog.builder(this)
                ...
                .bodyTitle("Tipo di provento")
                .showTitleInBody(true)
                .theme()
                .type(AppDialog.DialogType.INFO)
                .message("Decidi a quale tipologia di provento appartiene il movimento di deposito.")
                .details("Come vuoi classificare il movimento?")
        ...
        descrizione = result.getActionId();
        ...
        if (IDsTr.size() == 1) {//La faccio solo se seleziono il singolo movimento
            ...
            if (!ListaIDMovimentiUguali.isEmpty()) {
                ...
                String details = "Vuoi applicare la stessa classificazione anche agli altri movimenti simili?";
    ...
    private boolean Crea_CashoutCommissioni(Set<String> IDsTr, String Note) {
        ...
        String descrizione;
        String dettaglio = "PCO - Cashout, acquisti con crypto etc.. (plusvalenza)";
        ...
        AppDialog.DialogResult result = AppDialog.builder(this)
                ...
                .theme()
                .type(AppDialog.DialogType.INFO)
                .message("Decidi a quale tipologia appartiene il movimento di deposito.")
                .details("Come vuoi classificare il movimento?")
                .action(AppDialog.DialogAction.builder("cancel", "Annulla")
                        .role(AppDialog.ActionRole.SECONDARY)
                        .build())
                .action(AppDialog.DialogAction.builder("CASH OUT", "CASH OUT")
                        .role(AppDialog.ActionRole.NEUTRAL)
                        .build())
                .action(AppDialog.DialogAction.builder("COMMISSIONI", "COMMISSIONI")
                        .role(AppDialog.ActionRole.NEUTRAL)
                        .build())
                .showDialog();
    ...
    private boolean Crea_DonazioneFurto(Set<String> IDsTr, String Note) {
        //Può essere un movimento multiplo
        //può essere un movimento multiplo
        ...
        AppDialog.DialogResult result = AppDialog.builder(this)
                ...
                .theme()
                .type(AppDialog.DialogType.WARNING)
                .message("Scegli come classificare il movimento.")
                .details("Opzioni disponibili: furto, donazione o burn.")
                .action(AppDialog.DialogAction.builder("cancel", "Annulla")
                        .role(AppDialog.ActionRole.SECONDARY)
                        .build())
                .action(AppDialog.DialogAction.builder("furto", "FURTO")
                        .role(AppDialog.ActionRole.NEUTRAL)
                        .build())
                .action(AppDialog.DialogAction.builder("donazione", "DONAZIONE")
                        .role(AppDialog.ActionRole.NEUTRAL)
                        .build())
                .action(AppDialog.DialogAction.builder("burn", "BURN")
                        .role(AppDialog.ActionRole.NEUTRAL)
                        .build())
                .showDialog();
     ...
     private boolean Crea_Trasferimenti(Set<String> IDsTr, String descrizione,String Note) {
        if (Tabella_MovimentiAbbinati.getSelectedRow() >= 0&&IDsTr.size()==1) {
        ...
        Messaggi.WarningMessage("Attenzione richiesta!", 
                "Il movimento selezionato dalla tabella è già abbinato ad un altro movimento!<br>"
                + "Scegliere altro movimento oppure disabbinare dal movimento precedentemente associato e ripetere la procedura.", this);

                        return false;
                    }
                } else {
        Messaggi.WarningMessage("Attenzione richiesta!", 
                "Per questo tipo di movimentazione è obbligatorio selezionare il movimento corrispondente nella tabella!", this);
                    return false;
                }
    }
...
// qui prosegue la tua logica attuale
            ...
            if (Principale.Funzioni_isNumeric(m, false)) {
                ...
                Messaggi.WarningMessage("Numero non valido",
                        "Attenzione, " + m + " non è un numero valido!", this);
            }
        }
...
//        Importazioni.RiempiVuotiArray(MT2);
    ...
    public static Set<String> RiportaTransazioniASituazioneIniziale(Map<String,String[]> PartiConvolte){
        ...
        for(String IDs:PartiConvolte.keySet()){
        String IDPartiConvolte[]=PartiConvolte.get(IDs);
        String IDRitorno;       
        IDRitorno=RiportaTransazioniASituazioneIniziale(IDPartiConvolte,IDs);
        //IDRitorno è uguale all'id originale se non sono state fatte modifiche all'id altrimenti è il nuovo id del movimento
        //può essere diverso solo nel caso di scambi differiti
        IDsRitorno.add(IDRitorno);
        }
...
public static String RiportaTransazioniASituazioneIniziale(String IDPartiConvolte[],String IDOri){
        ...
        //Faccio il ripristino vero e proprio
        for (String ID : IDPartiConvolte) {
            ...
            if (attuale != null) {
                ...
                //Come ultima cosa controllo se il movimento aveva un vecchio prezzo salvato e lo ripristino
                //Succede se sono dei movimenti successivamente classificati come Acquisto o donazione
                if (attuale[35] != null && !attuale[35].isBlank()) {
                    attuale[15] = attuale[35];
                    attuale[35] = "";
                }
